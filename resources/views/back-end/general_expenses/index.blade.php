		                			...
		                			@endphp
					                	<tr>
											...
											@if(Auth::user()->can('edit-general-expense') || $isAdministrator)
												...
												@endif
											@endif
											@if(Auth::user()->can('receipt-expense') || $isAdministrator)
												@if(Auth::id() != $item->created_by)
													<a class="btn btn-sm btn-success" style="opacity: 0.8; cursor: not-allowed;" title="Edit" href="#">{{ __('item.receipt') }}</a>
												@else
													<a class="btn btn-sm btn-success" title="Edit" href="{{ route('general_expense.receipt_expense',['id' => $item->id]) }}">{{ __('item.receipt') }}</a>
												@endif
											@endif
