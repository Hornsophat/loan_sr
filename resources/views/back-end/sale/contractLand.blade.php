					                		...
					                		<td>
								                 <ul>
											            	...
											            	<table width="100%">
												            	...
												            	<tr>
																	<td width="15.2%" style="padding: 0px;"><p class="p">&emsp;&emsp;&emsp;- មានទំហំក្បាលដី <b>{{$sale->land_num}}</b> ម៉ែត្រ បណ្តោយ <b>{{$sale->address_zip_code}}</b> ម៉ែត្រ និងទំហំផ្ទះ <b>{{$sale->width }} </b> ម៉ែត្រ X <b>{{$sale->length}}</b> ម៉ែត្រ។</p></td>
									...
									?>
														...
														if($sale->deposit<=0)
														{
															...
															?>
																<tr>
																	<td align="left" style="font-family: 'Khmer OS system">
																&emsp;&emsp;​​​​​​​​​​ <b>៤.១.{{AppHelper::khMultipleNumber($i)}}. ដំណាក់កាលទី {{AppHelper::khMultipleNumber($i)}}</b>  ៖ ភាគី “ខ” យល់ព្រមបង់ប្រាក់កក់ចំនួន $  <b>{{ number_format( $reservation->amount,2 )}}</b><b>(​{{ AppHelper::khNumberWord( $reservation->amount) }}) ដុល្លារអាមេរិកគត់</b> ឲ្យទៅភាគី “ក” នៅថ្ងៃទី<b>{{AppHelper::khMultipleNumber(date('d', strtotime($reservation->date_booked)))}} </b>ខែ <b>{{AppHelper::khMonth(date('m', strtotime($reservation->date_booked)))}}</b>  ឆ្នាំ <b>{{AppHelper::khMultipleNumber(date('Y', strtotime($reservation->date_booked)))}}</b>។
																	</td>
																</tr>
																<!-- get book details -->
																<?php
																	foreach($book_details as $book)
																	{
																		...
																		?>
																			<tr>
																				<td align="left" style="font-family: 'Khmer OS system">
																			&emsp;&emsp;​​​​​​​​​​ <b>៤.១.{{AppHelper::khMultipleNumber($i)}}. ដំណាក់កាលទី {{AppHelper::khMultipleNumber($i)}}</b>  ៖ ភាគី “ខ” យល់ព្រមបង់ប្រាក់កក់ចំនួន $  <b>{{ number_format ( $book->amount,2) }}</b><b>(​{{ AppHelper::khNumberWord( $book->amount) }}) ដុល្លារអាមេរិកគត់</b> ឲ្យទៅភាគី “ក” នៅថ្ងៃទី<b>{{AppHelper::khMultipleNumber(date('d', strtotime($book->date)))}} </b>ខែ <b>{{AppHelper::khMonth(date('m', strtotime($book->date)))}}</b>  ឆ្នាំ <b>{{AppHelper::khMultipleNumber(date('Y', strtotime($book->date)))}}</b>។
																				</td>
																			</tr>
																		<?php
														...
														foreach($loans as $loan)
														{
															...
															if($loan_count>$i)
															{
																...
																<tr>
																	<td align="left" style="font-family: 'Khmer OS system">
																&emsp;&emsp; <b>៤.១.{{AppHelper::khMultipleNumber($i)}}. ដំណាក់កាលទី {{AppHelper::khMultipleNumber($i)}}</b> ៖ ភាគី “ខ” យល់ព្រមបង់ប្រាក់បន្ថែមចំនួន​ $ <b>{{ number_format( $loan->loan_amount,2 ) }}</b> <b>(​{{ AppHelper::khNumberWord($loan->loan_amount) }}) ដុល្លារអាមេរិកគត់</b> ឲ្យទៅភាគី “ក” នៅថ្ងៃទី<b>{{AppHelper::khMultipleNumber(date('d', strtotime($loan->loan_date)))}} </b>ខែ <b>{{AppHelper::khMonth(date('m', strtotime($loan->loan_date)))}}</b>  ឆ្នាំ <b>{{AppHelper::khMultipleNumber(date('Y', strtotime($loan->loan_date)))}}</b>។
																	</td>
																</tr>
															<?php
															}else{
													...
													if($child_item!==[])
													{
														...
														if($sale->deposit<=0)
														{
															...
															?>
																<tr>
																	<td align="left" style="font-family: 'Khmer OS system">
																&emsp;&emsp;​​​​​​​​​​ <b>១.{{AppHelper::khMultipleNumber($i)}}. ដំណាក់កាលទី {{AppHelper::khMultipleNumber($i)}}</b>  ៖ ភាគី “ខ” យល់ព្រមបង់ប្រាក់កក់ចំនួន $  <b>{{ number_format( $reservation->amount,2 )}}</b><b>(​{{ AppHelper::khNumberWord( $reservation->amount) }}) ដុល្លារអាមេរិកគត់</b> ឲ្យទៅភាគី “ក” នៅថ្ងៃទី<b>{{AppHelper::khMultipleNumber(date('d', strtotime($reservation->date_booked)))}} </b>ខែ <b>{{AppHelper::khMonth(date('m', strtotime($reservation->date_booked)))}}</b>  ឆ្នាំ <b>{{AppHelper::khMultipleNumber(date('Y', strtotime($reservation->date_booked)))}}</b>។
																	</td>
																</tr>
																<!-- get book details -->
																<?php
																	foreach($book_details as $book)
																	{
																		...
																		?>
																			<tr>
																				<td align="left" style="font-family: 'Khmer OS system">
																			&emsp;&emsp;​​​​​​​​​​ <b>១.{{AppHelper::khMultipleNumber($i)}}. ដំណាក់កាលទី {{AppHelper::khMultipleNumber($i)}}</b>  ៖ ភាគី “ខ” យល់ព្រមបង់ប្រាក់កក់ចំនួន $  <b>{{ number_format ( $book->amount,2) }}</b><b>(​{{ AppHelper::khNumberWord( $book->amount) }}) ដុល្លារអាមេរិកគត់</b> ឲ្យទៅភាគី “ក” នៅថ្ងៃទី<b>{{AppHelper::khMultipleNumber(date('d', strtotime($book->date)))}} </b>ខែ <b>{{AppHelper::khMonth(date('m', strtotime($book->date)))}}</b>  ឆ្នាំ <b>{{AppHelper::khMultipleNumber(date('Y', strtotime($book->date)))}}</b>។
																				</td>
																			</tr>
																		<?php
														...
														foreach($loans as $loan)
														{
															...
															if($loan_count>$i)
															{
																...
																<tr>
																	<td align="left" style="font-family: 'Khmer OS system">
																&emsp;&emsp; <b>១.{{AppHelper::khMultipleNumber($i)}}. ដំណាក់កាលទី {{AppHelper::khMultipleNumber($i)}}</b> ៖ ភាគី “ខ” យល់ព្រមបង់ប្រាក់បន្ថែមចំនួន​ $ <b>{{ number_format( $loan->loan_amount,2 ) }}</b> <b>(​{{ AppHelper::khNumberWord($loan->loan_amount) }}) ដុល្លារអាមេរិកគត់</b> ឲ្យទៅភាគី “ក” នៅថ្ងៃទី<b>{{AppHelper::khMultipleNumber(date('d', strtotime($loan->loan_date)))}} </b>ខែ <b>{{AppHelper::khMonth(date('m', strtotime($loan->loan_date)))}}</b>  ឆ្នាំ <b>{{AppHelper::khMultipleNumber(date('Y', strtotime($loan->loan_date)))}}</b>។
																	</td>
																</tr>
															<?php
															}else{
