									 ...
									 $effectiveDate = date('Y-m-d', strtotime("+$loanterm month", strtotime($datefirst)));						
						               ...
						               <!-- <tr style="text-align: center;">
								                ...
								                <p class="p"​><b>&emsp;&emsp;&emsp;១.២</b>
											        <ul>
											            <li><b>{{$property_type_group}}&emsp;&emsp;ការសងប្រាក់ដើម-ការប្រាក់ និង អត្រាការប្រាក់</b><br>
											            	<table width="100%">
											            		<tr>
												            		<td width="15.2%" style="padding: 0px;"><p class="p">&emsp;&emsp;&emsp;- ការសងប្រាក់ដើម ៖ ភាគី(ខ) មានកាតព្វកិច្ចសងការប្រាក់ និងប្រាក់ដើមវិញនៅរយ:ពេល <b>{{ $loan->installment_term }}</b> ខែ &emsp;&emsp;&emsp; ​<br> &emsp;&emsp;&emsp; ​ គិតចាប់ពីថ្ងៃដែលបានទទួលប្រាក់ពី ភាគី(ក)។</p></td>
												            	</tr>
												            	<tr>
										...
										&emsp;&emsp;&emsp; ក្នុងករណីដែលភាគី(ខ)មិនបានអនុវត្តតាមកិច្ចសន្យាសងប្រាក់អោយភាគី(ក) មានដូចក្នុងលក្ខខណ្ឌរួមខាងលើនោះភាគី(ខ)យល់ព្រមតាមការអនុវត្តដូចខាងក្រោម៖
						                		...
						                		<td>
						                			<p class="p"><b>&emsp;&emsp;&emsp;២.៣</b> 
						                				<ul>
															...
															<tr>
															<td align="left" style="font-family: 'Khmer OS system">
															&emsp;&emsp;&emsp; ករណីដែលភាគី(ខ) មិនសងបំណុលតាមការកំណត់ដូចមានក្នុងកិច្ចសន្យាហើយ ភាគី(ក)យល់ឃើញថាបំណុលនោះនិងត្រូវបាត់បង់ នោះភាគី(ខ)យល់ព្រមឲ្យភាគី(ក)រំលាយកិច្ចសន្យានេះមុនកាលកំណត់ដោយ ឯកតោភាគី និង ឥតលក្ខខណ្ឌ។ក្នុងករណីនេះ ភាគី(ខ)មានកាតព្វកិច្ចសងប្រាក់ទៅភាគី(ក)វិញទាំងអស់រួមមាន៖<br><b>ប្រាក់ដើមនៅសល់ + ការប្រាក់ + ប្រាក់ពិន័យ<b> ដែលភាគី(ខ)នៅជំពាក់។
															</td>
															</tr>
									                	</ul>
									                </p>
											...
											<td>
												<p class="p"><b>&emsp;&emsp;&emsp;២.៨</b> 
													<ul>
														<li><b>&emsp;&emsp;ការទទួលខុសត្រូវលើទ្រព្យបញ្ចាំ</b>
														</li>
														<tr>
														<td align="left" style="font-family: 'Khmer OS system">
														&emsp;&emsp;&emsp; - ភាគី(ខ) ត្រូវធានាទទួលខុសត្រូវលើទ្រព្យសម្បត្តិរបស់ខ្លួនដែលបានដាក់បញ្ចាំឲ្យភាគី(ក) ថាមានតម្លៃស្មើ ឬលើសពីទឹកប្រាក់ដែលបានខ្ចី ហើយធានាថាពិតជាកម្មសិទ្ធិរបស់ខ្លួនពិតប្រាកដមែន ដោយមិនមានភាគីទី(៣) ណាមកអះអាងថាជារបស់ខ្លួន។ បើផ្ទុយពីការធានានេះ ភាគី(ខ)ត្រូវទទួលខុសត្រូវចំពោះមុខច្បាប់ជាធរមាន។<br>
														&emsp;&emsp;&emsp; - ក្នុងករណីភាគី(ខ) ដាក់បញ្ចទ្រព្យសម្បត្តិជាចលនទ្រព្យ ហើយភាគី(8)នៅកំពុងប្រើប្រាស់ចលនទ្រព្យជាប្រចាំថ្ងៃនោះភាគី(ខ)ត្រូវយកចលនទ្រព្យដែលបានដាក់បញ្ចាំមកឲ្យភាគី(ក)ត្រួតពិនិត្យជារៀងរាល់មួយខែម្តង ដើម្បីធានាថាពិតជា មិនមានការខូចខាត ឬបាត់បង់ចលនទ្រព្យនេះឡើយ។ បើផ្ទុយពីការធានាខាងលើ ភាគី(ខ)ត្រូវទទួលខុសត្រូវចំពោះការបាត់បង់ឫការខូចខាតទាំងស្រុង។
														</td>
														</tr>
													</ul>
												</p>
										...
										if($child_item!==[])
										{
												foreach($child_item as $c)
											{
												...
												//  var_dump($c->property_name);
												?>
												<tr>
													<td style="border-right:1px solid black !important;font-size:15px;font-family:Khmer OS System"> <br>&emsp;&emsp;&emsp;ដីទំហំ<b> {{ $c->ground_surface }}</b> ម៉ែត្រការេ​ មានទីតាំងស្ថិតនៅភូមិ​ <b>{{ $c->village }}</b> <br>&emsp;&emsp;&emsp; ឃុំ/សង្កាត់ <b>{{ $c->commune }}</b> ស្រុក/ខណ្ឌ <b>{{ $c->district }}</b> ខេត្ត/ក្រុង <b>{{ $c->province }}</b>។ <br>&emsp;&emsp;&emsp; មានវិញ្ញាប័នប័ត្រសំគាល់ម្ចាស់អចលនវត្ថុលេខ <b>{{ $c->property_no }}</b>។</td>
													<td style="border-right:1px solid black !important;text-align:center;font-size:15px;font-family:Khmer OS System">1</td>
													<td style="border-right:1px solid black !important;text-align:center;font-size:15px;font-family:Khmer OS System">${{$c->property_price}}</td>
												</tr>		
												<?php
													...
													?>
													<td style="border-right:1px solid black !important;font-size:15px;font-family:Khmer OS System">&emsp;&emsp;១-ប្រភេទទ្រព្យសម្បត្តិ​​: <br>&emsp;&emsp;&emsp; ១.​ ដីទំហំ<b> {{ $sale->gf }}</b> ម៉ែត្រការេ​ មានទីតាំងស្ថិតនៅភូមិ​ <b>{{ $sale->village }}</b> <br>&emsp;&emsp;&emsp; ឃុំ/សង្កាត់ <b>{{ $sale->commune }}</b> ស្រុក/ខណ្ឌ <b>{{ $sale->district }}</b> ខេត្ត/ក្រុង <b>{{ $sale->province }}</b>។ <br>&emsp;&emsp;&emsp; មានវិញ្ញាប័នប័ត្រសំគាល់ម្ចាស់អចលនវត្ថុលេខ <b>{{ $sale->p_pn }}</b></td>
													<td style="border-right:1px solid black !important;text-align:center;font-size:15px;font-family:Khmer OS System"> 1</td>
												...
												<div class="content">
													...
													<tbody>
														<tr>
															...
															<tr><td><p></p></td></tr>
															<tr>
																<td style="text-decoration: underline;font-family: 'Khmer OS Muol Light';font-size:20px">លិខិតទិញ-លក់ដី</td>
															</tr>
															<tr>
																<td>
																	<p class="p" style="text-align: justify;">&emsp;&emsp;&emsp; ភាគីអ្នកលក់៖ ឈ្មោះ <span style="font-family: 'Khmer OS Muol Light';"><b>{{$sale->customer_name}}</b></span> ភេទ <b>{{$sale->customer_gender}} </b>កើតឆ្នាំ <b>{{AppHelper::khMultipleNumber(date('Y', strtotime($sale->customer_date_of_birht)))}} </b>សញ្ជាតិ <b>{{$sale->customer_nationality}}</b>  មានអត្តសញ្ញាណប័ណ្ណសញ្ជាតិខ្មែរលេខៈ <b>{{AppHelper::khMultipleNumber((string)$sale->customer_identity)}}</b>  ចុះថ្ងៃទី  <b>{{AppHelper::khMultipleNumber(date('d', strtotime($sale->cs_ident)))}} </b> ខែ  <b>{{AppHelper::khMultipleNumber(date('m', strtotime($sale->cs_ident)))}} </b>  ឆ្នាំ <b>{{AppHelper::khMultipleNumber(date('Y', strtotime($sale->cs_ident)))}} </b>អាស័យដ្ឋានបច្ចុប្បន្ន ផ្ទះលេខ <b>{{ $sale->cs_house_n}}</b>  ភូមិ <b> {{$sale-> vil_kh}}  </b> ឃុំ/សង្កាត់ <b>{{$sale->com_kh}}</b> ស្រុក/ខណ្ឌ <b>{{$sale->dis_kh}} </b>ក្រុង/ខេត្ត <b>{{$sale->prov_name}}</b> លេខទូរស័ព្ទ <b>{{ $sale->phone1 }}</b> បានព្រមព្រៀងលក់ដី.......ចំនួន......កន្លែង មានក្បាលដីលេខ..............  បណ្តោយ...........ម៉ែត្រ   ទទឹង..........ម៉ែត្រ ទំហំសរុបចំនួន <b>{{ $sale->gf }}</b> ម៉ែត្រការ៉េ សិ្ថតក្នុង ភូមិ <b>{{ $sale->village }}</b> ឃុំ/សង្កាត់ <b>{{ $sale->commune }}</b>  ស្រុក/ខណ្ឌ <b>{{ $sale->district }}</b> ខេត្ត/ក្រុង <b>{{ $sale->province }}</b> ។ 
																</td>
															</tr>
															<tr>
																<td><p class="pull-left" style="font-size:17px;font-family:Khmer OS System"> មានព្រំប្រទល់ជាប់នឹង៖</p></td>
															</tr>
															<tr>
																<td><p class="pull-left" style="font-size:17px;font-family:Khmer OS System">- ខាងជេីងទល់នឹង <b>{{ $sale->boundary_north }}</b>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;​&emsp;​&emsp;​- ខាងត្បូងទល់នឹង <b>{{ $sale->boundary_south }} </b></p></td>
															</tr>
															<tr>
																<td><p class="pull-left" style="font-size:17px;font-family:Khmer OS System">- ខាងកើតទល់នឹង <b>{{ $sale-> boundary_east}}</b>&emsp;&emsp;&emsp;&emsp;&emsp; - ខាងលិចទល់នឹ <b>{{ $sale->boundary_west }} </b></p></td>
															</tr>
															<tr>
																<td><p class="pull-left" style="font-size:17px;font-family:Khmer OS System">- លេខលិខិតកម្មសិទ្ធ <b>{{ $sale->p_pn }}</b> ចុះថ្ងៃទី.......ខែ........ឆ្នាំ.......... </p></td>
															</tr>
															<tr>
																<td><p class="pull-left" style="font-size:17px;font-family:Khmer OS System">- សមត្ថកិច្ចចេញលិខិត........................................</p> </td>
															</tr>
														
																<tr>
																	<td><p class="text-left" style="font-size:18px;font-family:Khmer OS System">ឲទៅអ្នកទិញ ឈ្មោះ យុគ ចិត្ត្រា ភេទ ប្រុស មុខរបរបច្ចុប្បន្ន រកស៊ី មានអត្តសញ្ញាណប័ណ្ណ សញ្ញាតិខ្មែរលេខ 011024552 ចុះថ្ងៃទី ០៥​ ខែ ០៨ ឆ្នាំ ២០១៥ ជាតំណាងឲក្រុមហ៊ុន ភី អ៊ីម ប៊ី (PMB) អាស័យដ្ឋានបច្ចុប្បន្ន ផ្ទះលេខ ៥៦CE0 ផ្លូវលេខ ១២២ ភូមិ​ ៩ សង្កាត់ផ្សារដេប៉ូទី៣ ខណ្ឌទួលគោក រាជធានីភ្នំពេញ។ <br>  ក្នុងតម្លៃ USD <b>{{$sale->p_pp}}</b>  ចំនួនទឹកប្រាក់ជាអក្សរ <b> (&nbsp;​{{ AppHelper::khNumberWord($sale->p_pp) }}</b> ដប់ពាន់ដុល្លារអាមេរិកគត់	)</p></td>
																</tr>
																<tr>
