...
class SalePropertyController extends Controller
{
    ...
    function sale_payment_receipt($id){
        ...
        if(!is_null($payment_transaction->reservation_id)){
            ...
            ->first();
            $payment_schedule_paid = PaymentSchedule::where([
                ['sale_id', '=', $payment_schedule->sale_id]
            ])->sum('paid');
            // var_dump(  $payment_schedule_paid);


            $day_penalty = Input::get('day_penalty',false);
            return view('back-end.sale_property.receipt_payment', compact('address','day_penalty','property','sale_item', 'payment_schedule', 'loan','customer', 'project', 'created_by', 'payment_transaction','payment_schedule_next','payment_schedule_paid'))->with('can_redirect_back', Request()->back);
        }
    }
    ...
    function get_payment_stage_amount(Request $request){
        ...
        if($request->payment_stage){
            if($request->payment_stage!='pay_full'){
                $payment_stage = PaymentStage::find($request->payment_stage);
                if($payment_stage){
                    $data['amount'] = $payment_stage->amount*1;
                }
            }
        }
        return $data;
